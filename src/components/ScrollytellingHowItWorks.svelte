<script lang="ts">
  ...
  const steps: Step[] = [
    {
      ...
      description: 'Desliza entre categorías y descubre miles de opciones. Restaurantes, tiendas, supermercados y productos del campo.',
      highlight: 'Carrusel intuitivo',
      color: '#E1C78E',
      image: '/img/captura 1.png'
    },
    {
      id: 'ai-search',
      number: '02',
      ...
      description: 'Mantén presionado y dile qué buscas. Nuestra IA conversacional entiende exactamente lo que necesitas.',
    ...
    },
    {
      ...
      title: 'Recibe',
      subtitle: 'en minutos',
      description: 'Confirma tu pedido y rastréalo en tiempo real. Tu orden llega a tu puerta en minutos.',
  ...
  let activeStep = $derived(Math.min(steps.length - 1, Math.floor(sectionProgress * steps.length)));

  // Progress within current step (0 to 1)
  let stepProgress = $derived(() => {
    const stepSize = 1 / steps.length;
    const stepStart = activeStep * stepSize;
    return Math.min(1, Math.max(0, (sectionProgress - stepStart) / stepSize));
  });
...
<section class="scrollytelling" bind:this={sectionRef} id="como-funciona">
  ...
  <div class="section-bg">
    <div class="bg-gradient" style="--active-color: {steps[activeStep]?.color || steps[0].color}"></div>
  ...
  <div class="sticky-wrapper">
    <div class="sticky-content">
      ...
      <div class="phone-container">
        <div class="phone-glow" style="background: radial-gradient(ellipse at center, {steps[activeStep]?.color || steps[0].color} 0%, transparent 70%);"></div>
          ...
          {#each steps as step, index}
            ...
            >
              <div class="phone-frame">
                <img src={step.image} alt="Paso {step.number}" class="phone-screenshot" />
              </div>
            </div>
          {/each}
        </div>
...
<style>
  ...
  .timeline-progress {
    width: 100%;
    background: linear-gradient(180deg, #E1C78E 0%, #B2D69A 50%, #5A8467 100%);
    border-radius: 1px;
  ...
  .timeline-step.active .step-indicator {
    ...
    box-shadow: 0 0 20px color-mix(in srgb, var(--step-color) 50%, transparent);
  }
